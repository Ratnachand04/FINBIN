...
Table~\ref{tab:apilatency} reports the serving path. These are in-process ASGI
measurements with the database and cache stubbed, so they isolate routing,
handler execution and JSON serialisation; there is no network hop, no TLS and no
worker pool, and we label them accordingly rather than quoting them as
production latency.

\begin{table}[t]\centering\small
\caption{Serving-path latency, in-process ASGI transport with database and cache
stubbed, $200$ requests per endpoint after warm-up.}
\label{tab:apilatency}
\begin{tabular}{@{}lrrrr@{}}
\toprule
Endpoint & p50 (ms) & p95 (ms) & p99 (ms) & Payload \
\midrule
\texttt{/api/v1/system}        & $1.083$ & $2.587$ & $4.233$ & $58$\,B \
\texttt{/api/market/snapshot}  & $1.338$ & $1.877$ & $2.081$ & $79$\,B \
\texttt{/api/health}           & $1.381$ & $2.186$ & $2.608$ & $15$\,B \
\texttt{/metrics}              & $1.573$ & $3.084$ & $4.399$ & $3.9$\,kB \
\textbf{\texttt{/api/v1/signals/}} & $\mathbf{2.897}$ & $\mathbf{6.141}$ & $8.321$ & $59$\,kB \
\texttt{/openapi.json}         & $3.309$ & $4.234$ & $6.478$ & $37$\,kB \
\midrule
Median across endpoints        & $1.477$ & $2.835$ & --- & --- \
\bottomrule
\end{tabular}
\end{table}

The signals endpoint, which is the product surface, serves a $59$\,kB payload at
$2.9$\,ms p50 and $6.1$\,ms p95. Comparing against Table~\ref{tab:latency}, a
single LSTM inference at $49.7$\,ms p50 is roughly $17\times$ the entire
serialised HTTP response for the endpoint that would carry it, which is a second
argument against that member independent of its accuracy.
...
\paragraph{Benchmarks exclude network and live I/O.} Tables~\ref{tab:latency}
and \ref{tab:apilatency} measure feature construction, model inference, backtest
throughput and in-process serving latency. They do not measure latency over a
real network with TLS and a worker pool, WebSocket delivery, ingestion
throughput against live APIs, or GPU memory under concurrent load, all of which
require a running deployment with live credentials. Two endpoints were excluded
from Table~\ref{tab:apilatency}: one performs external I/O in its handler
($7.1$\,s probe) and one failed response-model validation against stubbed rows.
