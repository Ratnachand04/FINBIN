...
> **Papers:** [system architecture](paper/binfin_system.tex) &middot; [evaluation methods](paper/crypto_direction_evaluation.tex)
...
### Ablation: does the ensemble help?

Run with `python scripts/ablation_and_benchmark.py`. Full output in
[`docs/ablation_results.json`](docs/ablation_results.json).

| Asset | Best single | Ensemble (deployed 0.30/0.40/0.30) | vs best single |
|---|---:|---:|---:|
| BTC | linear **52.98%** | 52.74% | -0.24pp (p=0.81) |
| ETH | linear **50.91%** | 50.77% | -0.14pp (p=0.88) |
| DOGE | linear **52.90%** | 51.54% | -1.36pp (p=0.27) |

**The ensemble beats no single member on any asset.** The LSTM is the worst
member everywhere (50.29 / 50.62 / **48.89%**) yet carries the *highest*
deployed weight (0.40). Fitting the weights inverts that:

| | Statistical | Neural | Boosted |
|---|---:|---:|---:|
| Deployed | 0.30 | **0.40** | 0.30 |
| Fitted (BTC / ETH / DOGE) | 0.59 / 0.46 / 0.55 | **0.24 / 0.24 / 0.24** | 0.16 / 0.30 / 0.21 |

And the LSTM costs **920x** the inference latency of the linear model that beats
it (49.7 ms vs 0.054 ms p50). Compute is allocated inversely to value.

### Decision layer: the rule is inert

Run with `python scripts/decision_layer_study.py`.

| Asset | Candidates | Pass primary | Pass supporting | Pass risk | **Fired** |
|---|---:|---:|---:|---:|---:|
| BTC | 2,084 | 2 | 2 | 0 | **0** |
| ETH | 2,084 | 17 | 6 | 0 | **0** |
| DOGE | 1,397 | 2 | 2 | 0 | **0** |

The signal generator emits **zero signals** across the whole evaluation, even
with the sentiment gate removed (the most generous treatment). The confidence
gate is set at 0.75; the maximum P(up) the model ever produces is **0.798** on
BTC, and fewer than 1 day in 1,000 clears it. The few that do are killed by the
ATR risk filter, because high model confidence coincides with high volatility.

This is a threshold/distribution mismatch between two individually correct
layers -- invisible to any component-level test.

### Benchmarks

| Operation | p50 | p95 | p99 |
|---|---:|---:|---:|
| Scaler transform | 0.052 ms | 0.060 ms | 0.073 ms |
| Logistic inference | 0.054 ms | 0.066 ms | 0.134 ms |
| Boosted inference | 2.333 ms | 4.060 ms | 4.669 ms |
| **LSTM inference** | **49.685 ms** | **77.673 ms** | **118.739 ms** |

Feature construction 5.7 us/bar; backtest engine 293,479 bars/s; process RSS
1,448 MB with all three model families loaded.

Serving path (`python scripts/benchmark_api.py`), in-process ASGI with DB and
cache stubbed, 200 requests/endpoint:

| Endpoint | p50 | p95 | p99 | Payload |
|---|---:|---:|---:|---:|
| `/api/v1/system` | 1.083 ms | 2.587 ms | 4.233 ms | 58 B |
| `/api/market/snapshot` | 1.338 ms | 1.877 ms | 2.081 ms | 79 B |
| `/api/health` | 1.381 ms | 2.186 ms | 2.608 ms | 15 B |
| `/metrics` | 1.573 ms | 3.084 ms | 4.399 ms | 3.9 kB |
| **`/api/v1/signals/`** | **2.897 ms** | **6.141 ms** | 8.321 ms | 59 kB |

These isolate routing, handler execution and serialisation. No network hop, no
TLS, no worker pool — quoted with those conditions attached, not as production
latency.

**Deployment finding:** on this host PyTorch reports CUDA and enumerates the
RTX 4060, while TensorFlow reports zero GPUs (native-Windows GPU support was
dropped in TF >= 2.11). So the QLoRA path runs on GPU while the LSTM path
silently runs on CPU, on the same machine. Capability detection must be
**per-framework**, not per-host.
...
Two manuscripts live in [`paper/`](paper/). Both are Elsevier `elsarticle`
format; compile each with pdfLaTeX three times (bibliographies are inline, so
BibTeX is not needed).

**1. The system paper --- [`binfin_system.tex`](paper/binfin_system.tex)**
*BINFIN: A Self-Hosted, Reproducible Architecture for Multi-Modal Cryptocurrency
Signal Generation on Commodity Hardware.*

This is the paper about **this model**. It describes the architecture end to
end: the four-source ingestion layer, TimescaleDB storage, the dual-path
sentiment subsystem (quantised Mistral-7B with a FinBERT fallback), the
forecasting ensemble, rule-composed signal generation, the execution simulator,
the hybrid GPU/CPU split runtime with capability-based fallback, and the
verification harness. The predictive evaluation is reported honestly.

**2. The evaluation paper --- [`crypto_direction_evaluation.tex`](paper/crypto_direction_evaluation.tex)**
*Calibrate the Instrument, Then Check the Baseline: An Audited Re-Evaluation of
Daily Cryptocurrency Direction Forecasting.*

A companion methods paper. It documents the twelve evaluation faults found in an
earlier revision of this system, the disguised-label case study, and why a
zero-parameter baseline matches the model. Separable from the system paper and
citable on its own.

Every number in both is traceable to [`docs/evaluation_results.json`](docs/evaluation_results.json)
and [`docs/mutation_matrix.json`](docs/mutation_matrix.json), regenerated by
`scripts/evaluate_walkforward.py` and `scripts/mutation_matrix.py`.
...
| [`scripts/ablation_and_benchmark.py`](scripts/ablation_and_benchmark.py) | Ensemble ablation, fitted weights, latency/throughput benchmarks |
| [`scripts/decision_layer_study.py`](scripts/decision_layer_study.py) | Signal-generator funnel and confidence-gate diagnostics |
| [`scripts/benchmark_api.py`](scripts/benchmark_api.py) | Serving-path latency with the data layer stubbed |
...
| [`paper/binfin_system.tex`](paper/binfin_system.tex) | System architecture paper (the paper about this model) |
| [`paper/crypto_direction_evaluation.tex`](paper/crypto_direction_evaluation.tex) | Companion methods paper on evaluation faults |
